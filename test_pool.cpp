...
public:
    TestPoolClass(int n, double period_ms)
    {
        for (int i = 0; i < n; i++)
        {
            std::shared_ptr<LiteTimer> tm = std::make_shared<LiteTimer>(period_ms, std::bind(&TestPoolClass::_callback, this, i));
            this->tms_.push_back(tm);
            this->tp_.addTimer(tm);
        }
